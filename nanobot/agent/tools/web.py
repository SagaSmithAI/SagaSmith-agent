...
# signature, or key query values) must never leave the machine. Matching is by
# parameter name: over-matching only costs the local readability fallback,
# while under-matching leaks a secret.
...
def _url_carries_credentials(url: str) -> bool:
    ...
    for name, _value in parse_qsl(query, keep_blank_values=True):
        ...
        if lowered in _CREDENTIAL_QUERY_PARAMS or lowered.startswith(
...
async def _stream_with_safe_redirects(
    ...
    for _ in range(MAX_REDIRECTS + 1):
        ...
        next_url = urljoin(str(response.url), location)
        chain_carries_credentials = chain_carries_credentials or _url_carries_credentials(
            next_url
        )
        is_valid, error_msg = _validate_url_safe(next_url)
        if not is_valid:
...
class WebFetchTool(Tool):
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        # in the observed redirect chain before the original URL may be sent to
        # Jina. Any preflight failure keeps the fetch on the local path.
