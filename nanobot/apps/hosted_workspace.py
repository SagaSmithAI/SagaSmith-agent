...
class HostedWorkspaceLease:
    ...
    def cleanup_registered(
        ...
        *,
        ...
        known: list[tuple[float, Path, dict[str, Any]]] = []
        active_count = 0
        for child in policy.root.iterdir():
            ...
            if value.get("status") == "active":
                active_count += 1
                continue
            accessed = float(value.get("last_access_at") or value.get("created_at") or 0)
            known.append((accessed, resolved, value))
        ...
        for path in sorted(set(remove), key=str):
            ...
            try:
                ...
                continue
            if current.get("schema") != MARKER_SCHEMA or current.get("status") != "terminated":
                continue
            shutil.rmtree(path)
            removed.append(path)
        return removed
