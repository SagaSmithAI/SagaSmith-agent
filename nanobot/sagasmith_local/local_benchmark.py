...
async def measure(executable: Path, root: Path, iterations: int) -> dict:
    ...
    config = MCPServerConfig(
        ...
        expose_resources_and_prompts=False, env={
            ...
            "SAGASMITH_DND_SKILLS_DIR": str(root / "skills"),
    ...
    async with AsyncExitStack() as stack:
        ...
        with request_context(RequestContext(channel="cli", chat_id="benchmark")):
            campaign = await call("campaign_create", {"name": "Disposable local benchmark"})
...
def main():
    ...
    with TemporaryDirectory(prefix="sagasmith-authority-benchmark-") as directory:
        result = asyncio.run(measure(args.dnd_python.resolve(), Path(directory), args.iterations))
