...
@pytest.mark.asyncio
async def test_generated_image_download_rejects_oversized_content_length(monkeypatch) -> None:
    monkeypatch.setattr(
        "nanobot.security.network.socket.getaddrinfo",
        _resolve_public,
    )
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, headers={"content-length": "9"}, content=PNG_BYTES)
...
@pytest.mark.asyncio
async def test_generated_image_download_rejects_non_image_body(monkeypatch) -> None:
    monkeypatch.setattr(
        "nanobot.security.network.socket.getaddrinfo",
        _resolve_public,
    )
    ...
    with pytest.raises(ImageGenerationError, match="supported image"):
        await _download_image_data_url(
            "https://cdn.example/image.png",
            transport=httpx.MockTransport(handler),
        )
...
@pytest.mark.asyncio
async def test_generated_image_download_redacts_http_error_body(monkeypatch) -> None:
    monkeypatch.setattr(
        "nanobot.security.network.socket.getaddrinfo",
        _resolve_public,
    )
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(500, content=b"provider-internal-secret")
...
@pytest.mark.asyncio
async def test_generated_image_download_enforces_redirect_limit(monkeypatch) -> None:
    monkeypatch.setattr(
        "nanobot.security.network.socket.getaddrinfo",
        _resolve_public,
    )
    ...
    with pytest.raises(ImageGenerationError, match="redirect limit"):
        await _download_image_data_url(
            "https://cdn.example/image.png",
            transport=httpx.MockTransport(handler),
        )

    assert len(requested) == image_generation._IMAGE_DOWNLOAD_MAX_REDIRECTS + 1
...
@pytest.mark.asyncio
async def test_proxied_generated_image_download_revalidates_redirects(monkeypatch) -> None:
    original_getaddrinfo = socket.getaddrinfo
