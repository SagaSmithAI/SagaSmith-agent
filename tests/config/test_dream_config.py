...
def test_dream_config_uses_model_override_name() -> None:
    cfg = DreamConfig.model_validate({"modelOverride": "openrouter/sonnet"})
...
def test_dream_config_rejects_retired_fields() -> None:
    for payload in ({"cron": "0 */4 * * *"}, {"model": "openrouter/sonnet"}):
        try:
            DreamConfig.model_validate(payload)
        except ValueError:
            continue
        raise AssertionError(f"retired config was accepted: {payload}")
