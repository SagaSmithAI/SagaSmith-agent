...
class TestHistoryWithCursor:
    ...
    def test_compact_history_preserves_unprocessed_backlog(self, tmp_path):
        store = MemoryStore(tmp_path, max_history_entries=2)
    ...
    def test_compact_history_keeps_recent_processed_context(self, tmp_path):
        ...
        entries = store.read_unprocessed_history(since_cursor=0)
        assert [entry["cursor"] for entry in entries] == [3, 4, 5, 6]
...
def test_history_jsonl_entries_are_read_by_cursor(tmp_path):
    store = MemoryStore(tmp_path)
    store.history_file.write_text(
        '{"cursor": 1, "timestamp": "2026-03-30 14:30", "content": "event"}\n',
        encoding="utf-8",
    )
