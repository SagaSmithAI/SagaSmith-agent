...
async def test_settings_api_returns_safe_subset_and_updates_whitelist(
    ...
    try:
        ...
        assert workspace_path == config.agents.defaults.workspace
...
def test_settings_payload_reports_workspace_sandbox(monkeypatch, tmp_path) -> None:
    ...
    save_config(config, config_path)
    monkeypatch.setattr("nanobot.config.loader._current_config_path", config_path)
    monkeypatch.setenv("NANOBOT_WORKSPACE_SANDBOX_ENFORCED", "macos_app_sandbox")
...
# typed envelopes (`new_chat` / `attach` / `message`).
...
async def test_multiplex_invalid_frames_return_error(bus: MagicMock) -> None:
    ...
    try:
        async with websockets.connect(f"ws://127.0.0.1:{port}/ws?client_id=bad") as client:
            ...
            # Invalid untyped input is rejected without closing the connection.
            await client.send("still-alive")
            err4 = json.loads(await client.recv())
            assert err4 == {
                "event": "error",
                "detail": "WebSocket frames must be typed JSON envelopes",
            }
...
def test_parse_envelope_rejects_untyped_and_malformed_frames() -> None:
