...
def test_control_events_live_on_the_typed_event_field() -> None:
    events = [
        RuntimeModelUpdatedEvent(model="gpt-5.5", model_preset="high"),
        GoalStateSyncEvent({"active": True}),
        GoalStatusEvent(status="running", started_at=1.25),
        TurnEndEvent(latency_ms=42, goal_state={"active": False}),
        SessionUpdatedEvent(scope="metadata"),
        RetryWaitEvent(content="waiting"),
    ]

    for event in events:
        msg = outbound_message_for_event(channel="websocket", chat_id="chat-1", event=event)
        assert outbound_event_from_message(msg) is event
